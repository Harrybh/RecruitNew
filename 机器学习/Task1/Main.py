...
'''
softmax 函数:将各个输出节点的输出值范围映射到[0, 1]，并且约束各个输出节点的输出值的和为1
为每个输出分类的结果都赋予一个概率值，表示属于每个类别的可能性。
'''
class NET(nn.Module):
...
# 选一个你喜欢的优化器
# 举个例子 SGD优化器optimizer = torch.optim.SGD(net.parameters(), lr=0.02)
...
for epoch in range(train_times):
    ...
    # 根据梯度下降法，每次反向传播前将累计的梯度清零 w=w-\alphad \frac{dJ(w)}{dw} 就是将其中累计的 \frac{dJ(w)}{dw} 清零
    optimizer.zero_grad()
    # 完善代码:反向传播 并更新所有参数
    loss.backward()
    optimizer.step()
...
'''
本次问题属于监督学习中的分类问题(应该是3分类)，batch size == 150 * 0.8 = 120 
本题采用的是将整个训练集作为一batch进行训练，对于大数据集来说是不适用
1. 数据集过大存在内存限制，运算也难以进行
2. batch size增大到一定程度，其确定的下降方向已经基本不再变化。
'''